...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        syncStepsButton.setOnClickListener { syncType = "steps"; loadPage("daily") }
        syncSleepButton.setOnClickListener { syncType = "sleep"; loadPage("sleep") }
        syncHeartRateButton.setOnClickListener { syncType = "hr"; loadPage("hr") }
        syncStressButton.setOnClickListener { syncType = "stress"; loadPage("stress") }
        syncCaloriesButton.setOnClickListener { syncType = "calories"; loadPage("daily") }
        syncSpO2Button.setOnClickListener { syncType = "spo2"; loadPage("daily") }
    }
                ...
                when {
                    ...
                    syncType == "calories" || syncType == "all" -> {
                        ...
                        if (calories != null) {
                            ...
                            runOnUiThread { statusText.text = "🔥 ${calories.totalCalories} kcal" }
                        }
                    }
                    syncType == "spo2" || syncType == "all" -> {
                        val spo2 = GarminDataParser.extractSpO2(json)
                        if (spo2 != null) {
                            healthWriter.writeSpO2(spo2)
                            runOnUiThread { statusText.text = "🫁 SpO2: ${spo2.avgSpO2}%" }
                        }
                    }
                }
